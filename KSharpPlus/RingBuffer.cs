...
public class RingBuffer<T> : ICollection<T> {
    ...
    public void CopyTo(T[] array, int index) {
        if (array.Length - index < 1) throw new ArgumentException("Target array is too small to contain the elements from this buffer.", nameof(array));

        int ci = 0;

        for (int i = CurrentIndex; i < InternalBuffer.Length; i++) array[ci++] = InternalBuffer[i];
        
        for (int i = 0; i < CurrentIndex; i++) array[ci++] = InternalBuffer[i];
    }

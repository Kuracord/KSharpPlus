...
public static class Optional {
    ...
    /// <returns>Created optional.</returns>
    public static Optional<T> FromNoValue<T>() => default;
}
...
public readonly struct Optional<T> : IEquatable<Optional<T>>, IEquatable<T>, IOptional {
    ...
    public bool IsDefined(out T? value) => (value = _val) != null;
    ...
    public static bool operator ==(Optional<T> opt1, Optional<T> opt2) => opt1.Equals(opt2);
...
/// <seealso cref="KuracordJson.Serializer" />
internal sealed class OptionalJsonContractResolver : DefaultContractResolver {
    protected override JsonProperty CreateProperty(MemberInfo member, MemberSerialization memberSerialization) {
        ...
        if (!type?.GetTypeInfo().ImplementedInterfaces.Contains(typeof(IOptional)) ?? true) return property;
        ...
        MemberInfo? declaringMember = property.DeclaringType?.GetTypeInfo().DeclaredMembers.FirstOrDefault(e => e.Name == property.UnderlyingName);
